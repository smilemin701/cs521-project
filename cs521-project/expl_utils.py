...
def shap_robust(X, label, pdt, model, n_sample, norm, eps_start, step_size, niters, device, X_, y_):
    X = X.cpu().numpy()
    Xs = np.repeat(X.reshape(1, -1), n_sample, axis=0)
    Xs_img = Xs.reshape(n_sample, 1, 28, 28)
    ...
    anchor_samples = Zs.reshape(n_sample, 1, 28, 28)
    print('label',label)
    robust_ub = evaluate_robustness(X_, y_, model, False, None, norm, anchor_samples, eps_start,
                                        step_size, niters, device, reverse_anchor=False)

    print('r:', robust_ub.shape)
    Zs_real = np.copy(Zs)
    Zs_real[Zs == 1] = Xs[Zs == 1]
    Zs_real_img = Zs_real.reshape(n_sample, 1, 28, 28)
    Zs_img = Variable(torch.tensor(Xs_img - Zs_real_img), requires_grad=False).float()
...
def greedy_as(X, y, model, targeted, target, norm, eps_start, step_size, niters, device, reverse_anchor, percentage=0.5):
    ...
    while cur_anchor.sum() < 784 * percentage:
        ...
        ks = np.ones(n_sample)
        feature_robust = kernel_regression(Zs, ks, robust_ub)
        print('feature_robust', feature_robust.shape)
        if reverse_anchor:
...
def saliency_expl(expl_method, Xs, ys, model, norm= None, eps_start= None, step_size=None, niters=None, device=None):
    """currently only suppor one example at a time"""
    print('Computings explanation by {}...'.format(expl_method))
    ...
    anchor_maps, _ = get_explanation_pdt(Xs, model, ys, expl_method, sg_r=sg_r, sg_N=sg_N, given_expl=given_expl, binary_I=False, label_ori = ys)
    anchor_maps = np.abs(anchor_maps)
    return anchor_maps


def get_explanation_pdt(image, model, label, exp, sg_r=None, sg_N=None, given_expl=None, binary_I=False, norm= None, eps_start= None, step_size=None, niters=None, device=None, label_ori = None):
    label = label[0]
    ...
    elif exp == 'SHAP_robust':
        expl = shap_robust(image.cpu(), label, pdt, model, 5000, norm, eps_start, step_size, niters, device, X_=image, y_=label_ori)
    elif exp == 'IG':
        for i in range(10):
